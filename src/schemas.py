...
class QuantResponse(BaseModel):
    probabilities: List[float] = Field(description="List of probabilities for major outcomes.", max_length=5)
    expected_value: str = Field(description="Calculated expected value or quantitative summary.")
    risk_factors: List[str] = Field(description="List of quantifiable risk factors.", max_length=5)
...
class StrategistResponse(BaseModel):
    ...
    adversarial_moves: List[str] = Field(description="Potential adversarial reactions or worst-case scenarios.", max_length=5)
...
class BehavioristResponse(BaseModel):
    cognitive_biases: List[str] = Field(description="Identified cognitive biases (e.g., sunk cost, availability heuristic).", max_length=5)
    blind_spots: List[str] = Field(description="Potential psychological blind spots in the user's framing.", max_length=5)
...
class JudgeResponse(BaseModel):
    ...
    actionable_next_steps: List[str] = Field(description="1-3 concrete, actionable next steps based on first principles.", max_length=3)
